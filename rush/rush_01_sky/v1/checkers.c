...
/*   Updated: 2022/02/14 16:28:24 by bbonaldi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	count_of_spaces(char *s,t_table_size s_tsize)
{
	...
	while (s[i])
	{
		if(!(valid_number(s[0], s_tsize.size) && valid_number(s[str_len - 1], s_tsize.size)))
			return (0);
		else if(s[i] == ' ')
			count++;
		i++;
...
int	is_valid_argument(char *s, t_table_size *s_tsize)
{
	...
	i = 0;
	numbers_count = 0;
	s_tsize->size = get_table_size(s, s_tsize->sides); // 4
	while (s[i])
	{
	...
	return ((numbers_count == s_tsize->size * s_tsize->sides) && count_of_spaces(s, *s_tsize));
}
